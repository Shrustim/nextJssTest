...
const SoftwareEdit: NextPage =   ({data}:any) => {
  console.log("data",data)
    const router = useRouter()
    const { softwareCode } = router.query
    const [isLoading,setIsLoading] = useState(false);
    const [softwarelist,setSoftwareslist] = useState<any>(data.SoftwareData ? data.SoftwareData : []);
     ...
     const getSoftwaresListbySoftwareCode= async()=>{
       if(userData.id){
        ...
        //    console.log(userData.id)
           try {
             const response: any = await apiobj.request("software/listbysoftwarecode", {"softwareCode":softwareCode}, "post");
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  var SoftwareData:any = [];
  var companyInfo:any = {};
  let softwareCode = context.params?.softwareCode;
  // Fetch data from external API
    try {
   
      const response: any = await apiobj.request("software/listbysoftwarecode", {"softwareCode":softwareCode}, "post" ,context.req.cookies['token']);
      SoftwareData = response.data.list
